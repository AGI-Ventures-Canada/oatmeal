...
export const importRoutes = new Elysia({ prefix: "/public/import" })
  ...
  .post(
    ...
    async ({ body, set }) => {
      ...
      const data = await extractEventPageData(normalizeUrl(body.url))
    ...
    {
      detail: {
        summary: "Preview event page data",
        description: "Fetches a public event page and extracts structured data for preview. No authentication required.",
        tags: ["public"],
      },
      body: t.Object({
...
export const dashboardImportRoutes = new Elysia({ prefix: "/dashboard/import" })
  ...
  .post(
    ...
    async ({ principal, body, set }) => {
      ...
      const { extractEventPageData } = await import("@/lib/services/event-page-import")
      const { extractEventPageRichContent } = await import("@/lib/services/luma-extract")
      const [eventData, richContent] = await Promise.all([
        extractEventPageData(body.url),
        extractEventPageRichContent(body.url),
      ])
    ...
    {
      detail: {
        summary: "Create hackathon from any event page URL",
        description: "Fetches a public event page, extracts structured data via schema.org markup, and creates a new hackathon. Requires hackathons:write scope.",
        tags: ["dashboard"],
      },
      body: t.Object({
        url: t.String({ minLength: 1 }),
