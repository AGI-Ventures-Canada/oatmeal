...
BEGIN
  ...
  WHERE hackathon_id = v_assignment.hackathon_id;

  IF ABS(v_weight_total - 1.0) > 0.0001 THEN
    RETURN QUERY SELECT FALSE, 'invalid_weight_total'::TEXT, 'Criteria weights must total 100%'::TEXT;
    RETURN;
  END IF;
  ...
  LOOP
    ...
    IF v_score_entry.score NOT IN (0, 1) THEN
      RETURN QUERY SELECT FALSE, 'score_out_of_range'::TEXT, 'Binary scores must be 0 or 1'::TEXT;
      RETURN;
    END IF;
...
BEGIN
  ...
  IF ABS(v_weight_total - 1.0) > 0.0001 THEN
    RETURN QUERY SELECT FALSE, 'invalid_weight_total'::TEXT, 'Criteria weights must total 100%'::TEXT, 0;
    RETURN;
  END IF;
  ...
  WITH scored_submissions AS (
    SELECT
      ...
      (SUM(s.score::numeric * jc.weight) / NULLIF(SUM(jc.weight), 0)) * 100 AS weighted_score
    FROM judge_assignments ja
