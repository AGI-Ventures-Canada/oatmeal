-- Role conflict guard: prevent a judge from registering as a participant or joining a team.
-- Mirrors the TypeScript checkRoleConflict() utility for defense-in-depth inside DB transactions.

-- 1. Update register_for_hackathon: block registration if user is a judge
...
BEGIN
  ...
  -- Role conflict guard: block if user is already a judge
...
-- 2. Update accept_team_invitation: block if user is a judge
...
BEGIN
  ...
  -- Role conflict guard: block if user is a judge
  PERFORM 1 FROM hackathon_participants
