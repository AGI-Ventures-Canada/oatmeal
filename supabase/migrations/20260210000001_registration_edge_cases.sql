...
BEGIN
  SELECT id, status, registration_opens_at, registration_closes_at, max_participants, ends_at
  INTO v_hackathon
  FROM hackathons
  ...
  IF v_hackathon.status IN ('draft', 'archived', 'completed', 'judging') THEN
  ...
  IF v_hackathon.ends_at IS NOT NULL AND v_now > v_hackathon.ends_at THEN
    RETURN QUERY SELECT FALSE, NULL::UUID, 'event_ended'::TEXT, 'This event has ended'::TEXT;
    RETURN;
  END IF;
  ...
  IF v_hackathon.registration_closes_at IS NOT NULL AND v_now > v_hackathon.registration_closes_at THEN
    RETURN QUERY SELECT FALSE, NULL::UUID, 'registration_closed'::TEXT, 'Registration has closed'::TEXT;
    RETURN;
  END IF;

  IF v_hackathon.registration_opens_at IS NULL AND v_hackathon.registration_closes_at IS NULL
     AND v_hackathon.status NOT IN ('registration_open', 'active') THEN
    RETURN QUERY SELECT FALSE, NULL::UUID, 'registration_not_open'::TEXT, 'Registration is not open'::TEXT;
    RETURN;
  END IF;
