...
const TokenSpan = ({ token }: { token: ThemedToken }) => (
  <span
    className="dark:!bg-[var(--shiki-dark-bg)] dark:!text-[var(--shiki-dark)] whitespace-pre"
    style={
      {
        color: token.color,
...
const LineSpan = ({
  ...
  <span className={cn(showLineNumbers ? LINE_NUMBER_CLASSES : "block", "whitespace-pre")} suppressHydrationWarning>
...
const CodeBlockBody = memo(
  function CodeBlockBody({
    ...
    return (
      <pre
        className={cn(
          "dark:!bg-[var(--shiki-dark-bg)] dark:!text-[var(--shiki-dark)] m-0 pl-0.5 pr-2 py-3 text-sm whitespace-pre",
          className
        )}
        style={preStyle}
        suppressHydrationWarning
...
export const CodeBlockContainer = ({
  ...
  <div
    className={cn(
      "not-fumadocs-codeblock not-prose group relative w-full overflow-hidden rounded-md border bg-background text-foreground",
      className
    )}
...
export const CodeBlockContent = ({
  ...
  // Memoized raw tokens for immediate display - always start with this for hydration consistency
