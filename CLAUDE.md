...
**CRITICAL: Every feature, action, and piece of content must be reachable in 3 navigation steps or fewer from any starting point. No exceptions.**

A "step" is any deliberate user interaction that changes what's on screen — a click, tap, keyboard shortcut, drill-down, or modal open. This applies to all user-facing flows — dashboard, public pages, mobile, and desktop:

- **Navigation depth**: No feature should be buried more than 3 levels deep. If a flow requires more than 3 steps, flatten the hierarchy or add shortcuts
- **Common actions**: Frequently used actions (create, edit, delete, view details) must be accessible in 1-2 steps from the relevant context
- **Settings and configuration**: All settings must be reachable in 3 steps from the dashboard home
- **Mobile parity**: The 3-step limit applies equally to mobile navigation — drill-down menus count as steps
- **Modals and dialogs**: Opening a modal counts as a step. Actions within the modal must complete within the remaining step budget
